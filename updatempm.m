...
% noGUI-update/install function for mpm-project via GITHUB (no graphical userinterface)
%% ___ INFO ___________________
% updatempm('info');     % display status information, (update status, last commit, last modified files)
% updatempm('changes');  % display last local modified files (compare last update with penultimate update) 
%% ___ UPDATES ___________________
% updatempm();   % check for updates, if available, user is asked to update via cmd-window
% updatempm(1);  % same as updatempm()
% updatempm(2) ; % just update (simplest version, user is not asked),
% updatempm(3) ; % hard reset, in case some local files have been modified/deleted
...
% updatempm(4,'path',LOCAL-PATH-TO-INSTALL-THE-TOOLBOX);
% ..replace "LOCAL-PATH-TO-INSTALL-THE-TOOLBOX" with yout destination-path
% updatempm(4,'path',pwd); % FRESH INSTALLATION to current directory
% or
% updatempm(4,'path',fullfile(pwd,'mpm'))
%
%% ___ ".git"-folder is missing/deleted ___________________
% updatempm(13)  ;% re-initiate .git
%


function varargout=updatempm(code,varargin)
...
if 0
    copyfile(fullfile(pwd,'updatempm.m'), fullfile(fileparts(pwd),'updatempm.m'),'f'  )
end
...
p.pathbefore=pwd;
if code~=4
    p.path=fileparts(which('mpmver'));
    if exist(p.path)==7
        cd(p.path);
    else
        disp('mpm-path not found');
        return
    end
end
%% ===============================================
...
if updatecode==1 %check before
    ...
    if isempty(msg);
        ...
        cprintf('*[0    0.4471    0.7412]','mpm-UPDATES FOUND - Update now?\n');
        cprintf('*[0    0.4471    0.7412]','================================\n');
...
if updatecode==4
    if isfield(p,'path')==0 | isempty(p.path)
        error('path for new installation not defined..see help of updatempm ');
        cd(p.pathbefore);
    end
    ...
    if strcmp(pa1,'mpm');%mpm-path selected
        patempup=pa2;
    else
        patempup=pa;
    end
...
%     end
    ...
    msgpafinal=[' DESTINATION PATH: ' fullfile(patempup,'mpm') ] ;
    if ispc==1
        msgpafinal=strrep(msgpafinal,filesep,[filesep filesep]);
    end
    cprintf([0 .5 0], [msgpafinal  '\n']);
    %% ==============[install]========================
    p2.mpmpath    =fullfile(patempup,'mpm');
...
gitpath=fileparts(which('mpmver.m'));
if isempty(gitpath)
    error('.git not found --- add mpm-path first');
end
...
if strcmp(trep,tloc)~=1
    cprintf('*[1 0 1]', ['UPDATE IS SUGGESTED!    type "updatempm(2)" to update toolbox'   '\n'] );
else
...
%% ========== [1] [mpm]-folder exists ==================
isPrevmpmdir=0;  % previous mpm dir exist
if exist(p2.mpmpath)==7
    isPrevmpmdir=1;
    % keyboard
    disp('..[mpm]-folder exist ..wait..folder will be renamed & removed');
    cd(fileparts(p2.mpmpath)); drawnow;
    pause(1);
    
    %disp(fullfile(antupd.patempup,'mpm'));
    %disp(fullfile(antupd.patempup,'_mpm'));
    fclose('all'); % remoclose all open file-handles
    
    if isunix && ~ismac
        system(['sudo rm -r "' p2.mpmpath '"']);
    else
        try;     rmdir(p2.mpmpath,'s'); end
    end
    %try; disp(pwd);    end
    drawnow;
end
...
if 1
    ...
    %% NOTE the installationPath without 'mpm' otherwise 'mpm' is within 'mpm'-dir
    %     if exist(p2.mpmpath)
    %         mkdir(p2.mpmpath);
    %     end
    %     cd(p2.mpmpath);
    ...
    if 1
        ...
        cprintf('*[0    0.4471    0.7412]','..CLONING mpm REPO from GITHUB..please wait...\n');
        %git clone https://github.com/pstkoch/mpm
        ...
        if ~isempty(strfind(msg,'unable to access')) || (exist(fullfile(p2.mpmpath,'.git'))~=7)
            if (exist(fullfile(p2.mpmpath,'.git'))~=7)
                disp('..failed to remove "mpm"-directory');
...
function [msg st]=mpm_gitstatus(message)
...
https='https://github.com/ChariteExpMri/mpm.git';
