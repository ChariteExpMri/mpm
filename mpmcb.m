%% mpmcb -internal functions
% mpmcb('version')  ;%




% not supported older stuff)
%% get selected files and folders
% fidi=mpmcb('getsel'); %get selected files/dirs
% fidi=mpmcb('getall'); %get all files/dirs
% fi  =mpmcb('getselstacked') ;%get files, stacked animalwise
% v=mpmcb('getanimals'); % get struct with animal-names, No slices, and FPdirs of selected animals 
...
% mpmcb('sel','tag','ok');
% mpmcb('sel','tag','issue|ok');
% ---------select based on string in fullpathFileName-----
% mpmcb('sel','filename',v);
% mpmcb('sel','filename','test_2figs\a1_001');
% ---------select string in FILEs-----
% mpmcb('sel','file','Nai|half');
% mpmcb('sel','file','Nai|half|a1');
% mpmcb('sel','file','a1_001');
% mpmcb('sel','file','all');  %select all files
% ---------select string in DIRs-----
% mpmcb('sel','dir','Nai|half');
% mpmcb('sel','dir','fside');
% mpmcb('sel','dir','all'); %select all dirs
% =============================================
% mpmcb('reload') ;% reload mpm

function varargout=mpmcb(varargin)

if 0
%     mpmcb('update')
%     [s1 s2]=mpmcb('getallsubjects');
%     mpmcb('close')
%     mpmcb('load')
%     mpmcb('load','F:\data3\histo2\data_Josephine\proj.m')
%     mpmcb('getsel')
%     mpmcb('getall')
%     mpmcb('sel')
%     mpmcb('updateListboxinfo');
end
...
hc=findobj(findobj(0,'tag','mpm'),'tag','txtversion');
if isempty(hc); return; end
version=mpmcb('version');
...
vstring=strsplit(help('mpmver'),char(10))';
idate=max(regexpi2(vstring,' \w\w\w 20\d\d (\d\d'));
dateLU=['mpm vers. ' char(regexprep(vstring(idate), {' (.*'  '  #\w\w ' },{''}))];
out=dateLU;
...
set(findobj(0,'tag','mpm'),'name',['mpm: ' projfile]);
...
try
    ...
    if exist(f2)==2
        hr=findobj(findobj(0,'tag','mpm'),'tag','currentImage');
        if isempty(hr)
